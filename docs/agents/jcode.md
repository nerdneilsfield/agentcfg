...
- `api_key_env` is native: the IR `api_key_env` maps directly and no secret is rendered.
- Provider `headers` are literal-only; IR `from_env` / `bearer_from_env` provider headers are rejected. Constant `value` headers are emitted.
...
`~/.jcode/mcp.json` holds `[server].<id>`-style entries with stdio-only support (`command`, `args`, `env`, `timeout_secs`, `enabled|disabled`). HTTP/SSE entries are parsed but skipped at load, so IR `http` transport is rejected. `${VAR}` / `${VAR:-default}` expansion is native for MCP string fields, so IR `from_env` renders as `"${VAR}"`. `Authorization.bearer_from_env` renders as `Bearer ${VAR}` inside headers. IR `timeout_ms` is emitted as `timeout_secs` (seconds, `ms/1000`); IR `cwd` has no jcode field and is rejected for MCP servers.
