...
| `fast-agent` | [evalstate/fast-agent](https://github.com/evalstate/fast-agent) | Chat · Responses · Anthropic | stdio + HTTP | Config + native model overlays + optional secrets fragment; whole-second MCP timeouts; disabled servers skipped |
| `crow` | [crow-cli/crow-cli](https://github.com/crow-cli/crow-cli) | Chat only | stdio + HTTP | `~/.agents/crow/config.yaml` fragment; default model ordered first; no provider headers or MCP `cwd`/timeout; disabled servers skipped |
