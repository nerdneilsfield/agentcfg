...
- 共支持 25 个 target，包括 fast-agent、Crow、Aider、Continue、Qwen Code 和 Kilo CLI。
- `gen --in-place` 更新原生配置；`gen --output` 写入指定文件或目录，支持多配置文件的 agent。
- 文件更新会替换 provider/model 和 MCP 区块，同步必要凭证及默认模型，保留无关设置。
- 校验错误会阻止生成；目标不支持的能力会给出 warning，其余可表示的配置继续生成。

用法及合并限制见[更新本地配置](#更新本地配置)。

设计原则：

- **显式配置凭据。** 使用带引号的字符串字面量或 `ENV:NAME`。
  agentcfg 不解析凭证环境变量引用；字面量按原值写入生成结果，引用则翻译成各 CLI
  的原生语法。
- **默认写 stdout。** `gen` 默认打印片段供审阅；指定 `--in-place` 或 `--output`
  可合并到原生配置文件。
